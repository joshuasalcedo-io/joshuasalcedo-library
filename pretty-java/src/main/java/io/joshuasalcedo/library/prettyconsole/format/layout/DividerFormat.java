...
public class DividerFormat implements VisualFormat {
    ...
    /**
     * Default width for dividers
     */
    private static final int DEFAULT_DIVIDER_WIDTH = 80;

    /**
     * Minimum number of line characters required on each side of text
     */
    private static final int MIN_LINE_CHARS = 4;

    /**
     * Margin for text truncation
     */
    private static final int TEXT_TRUNCATION_MARGIN = 6;

    /**
     * Spacing around text (1 space on each side)
     */
    ...
    public DividerFormat() {
        this(DEFAULT_DIVIDER_WIDTH, '─', ForegroundColor.WHITE, true);
    }
    ...
    public String format(String text) {
        ...
        if (text == null || text.isEmpty()) {
            ...
            // Ensure there's enough space for the text and at least minimum line characters
            if (textVisibleLength + MIN_LINE_CHARS > width) {
                // Text is too long, truncate it
                text = TextUtils.truncate(text, width - TEXT_TRUNCATION_MARGIN);
                textVisibleLength = TextUtils.visibleLength(text);
            }

            // Calculate the number of line characters on each side of the text
            int lineCharsPerSide = (width - textVisibleLength - TEXT_SPACING) / 2;
            int leftLineChars = lineCharsPerSide;
            int rightLineChars = width - textVisibleLength - TEXT_SPACING - leftLineChars;
